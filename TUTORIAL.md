...
| Klik Continue as Guest → `Failed to fetch dynamically imported module: .../src/pages/Dashboard.tsx` | Sesi dev server sedang **mati/restart** saat browser mem-fetch graf modul dashboard (sebelumnya ~100 modul dimuat sekaligus) | **Refresh halaman** setelah server kembali (klik **↻ Refresh preview**). Dashboard kini memuat tab secara **lazy** (per-tab), jadi beban awal jauh lebih kecil dan satu tab gagal tidak mem-bootstrap seluruh dashboard. |
